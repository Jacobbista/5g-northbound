...
async def test_retrieve_same_tenant_ok(client, make_token):
    # pkg-4471 is org=fiskarheden (conftest _TEST_ASSETS).
    resp = await client.post(RETRIEVE, json={"device": {"assetId": "pkg-4471"}},
                             headers=_hdr(make_token, org="fiskarheden"))
...
async def test_assets_list_filtered_by_org(client, make_token):
    # all seeded assets are fiskarheden -> a fiskarheden token sees all 3.
    same = await client.get(ASSETS, headers=_hdr(make_token, org="fiskarheden"))
