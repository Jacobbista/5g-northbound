...
def test_none_when_no_usable_fix():
    ...
    assert fuse_fixes([{"latitude": None, "longitude": None, "accuracy": 1.0}]) is None


def test_single_fix_passes_through_with_sources():
    out = fuse_fixes([{"latitude": 59.4, "longitude": 17.9, "accuracy": 0.5, "source": "wittra"}])
...
def test_sharper_source_dominates_and_radius_shrinks():
    fixes = [
        {"latitude": 0.0, "longitude": 0.0, "accuracy": 3.0, "sources": ["wifi"]},
        {"latitude": 1.0, "longitude": 1.0, "accuracy": 0.5, "sources": ["wittra"]},
    ]
...
def test_missing_source_drops_out_continuity():
    ...
    fixes = [
        {"latitude": None, "longitude": None, "accuracy": 3.0, "sources": ["wifi"]},
        {"latitude": 2.0, "longitude": 2.0, "accuracy": 0.5, "sources": ["wittra"]},
    ]
    out = fuse_fixes(fixes)
...
def test_latest_timestamp_and_union_altitude_from_sharpest():
    fixes = [
        ...
        {"latitude": 0.0, "longitude": 0.0, "accuracy": 0.5, "timestamp": "2026-01-01T00:00:05Z",
