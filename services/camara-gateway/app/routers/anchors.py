...
nominal tx power). The MEASURED RF model lives in the wifi-adapter
bindings (tx_power reference + path-loss n, fitted by the calibration DERIVE).
The demo talks only to the gateway, so the gateway proxies wifi-adapter's
...
async def anchors_calibration(_claims: dict = Depends(require_location_role)) -> dict[str, Any]:
    ...
    wifi-adapter is not wired or has no calibration yet."""
