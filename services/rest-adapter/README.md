...
`v0.1.0` - REST polling, schema persistence, in-process TTL cache, four auth schemes, dotted-path mapping with linear transforms. MQTT and webhook ingest paths are out of scope for this image.
...
4. Append the adapter to the engine's `ADAPTER_URLS` (e.g. `wittra=http://rest-adapter-wittra:8080`) and to `DEVICE_MAP` for the affected devices. Engine restart (or `kubectl rollout restart deployment/positioning-engine`) picks them up.
...
The compose stack ships a [`mock-wittra`](../../mocks/mock-wittra/) toy service that serves Wittra-shaped JSON without an account, plus a bind-mounted example schema. `make demo` is enough to see the full chain (engine → rest-adapter → mock-wittra) producing a CAMARA `Location` for `+390119876543` (the Wittra demo device).
...
curl -X PUT http://localhost:8092/schema \
  ...
  -d @rest-adapter/examples/wittra-schema.json
```

## What this image does NOT do

- **MQTT subscription.** Wittra recommends MQTT over REST for sensor data. A future `mqtt-adapter` image will follow the same operator workflow with a topic + payload schema, but REST polling is enough for an MVP demo.
- **Webhook ingestion.** No vendor here currently exposes a live webhook. When one does, the same translation logic lives in a sibling `webhook-adapter` image.
...
- [`mock-wittra/`](../../mocks/mock-wittra/) - local Wittra cloud fake used by the demo
