...
try:
    import yaml
except Exception:  # pragma: no cover - guarded; mock may ship without yaml
    yaml = None
...
def _caps() -> dict:
    ...
    (so the image self-declares with no env plumbing), and
    ADAPTER_CAPABILITIES (JSON) overrides/extends it. `make positioning-check`
