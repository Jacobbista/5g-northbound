...
Conventions for contributors and AI assistants working in this repository. Architecture, data contracts, deployment shape, and the adapter contract live under [`docs/`](docs/) — read those first; this file covers only how to write code that fits in cleanly.

## Layout

Each service is self-contained: its own `Dockerfile`, `requirements.txt` / `pyproject.toml`, `tests/`. Cross-service contracts cross the network, never a Python import. The four images are `camara-gateway`, `positioning-engine`, `wifi-positioning`, `positioning-demo`; see [`docs/deployment.md`](docs/deployment.md) for the build and publish flow.
...
- Configuration from environment variables via `pydantic_settings.BaseSettings`. No hardcoded URLs, secrets, or credentials in application code.
...
- The CAMARA OpenAPI documents under [`camara-gateway/spec/`](camara-gateway/spec/) are pinned to the meta-release recorded in `spec/VERSION`. Treat them as source of truth; do not hand-edit. To bump the pin: refetch from upstream at the new commit and update `VERSION`.
...
pytest                             # per-service from its own directory
npm test                           # in positioning-demo/
```
